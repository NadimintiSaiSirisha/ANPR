...
def eval(model, test_dirs):
    ...
    testloader = DataLoader(dst, batch_size=1, shuffle=True, num_workers=4)
    start = time()
...
def train_model(model, criterion, optimizer, num_epochs=25):
    for epoch in range(epoch_start, num_epochs):
        ...
        for i, (XI, Y, labels, ims) in enumerate(trainloader):
            ...
            # labels in train_model:  ('15_0_20_33_19_26_24', '0_0_28_24_25_20_33')
            ...
            print("YI: ", YI)
            # YI:  [[0, 0, 28, 24, 25, 20, 33], [0, 0, 1, 20, 33, 32, 27]]
            print("type(YI): ", type(YI))
            # type(YI):  <class 'list'>
            
            Y = np.array([el.numpy() for el in Y]).T  # Real value [cen_x,cen_y,w,h]
            print("Y: ", Y)
            # [[0.57152778 0.32112069 0.29583333 0.05948276]
            # [0.48472222 0.45474138 0.29166667 0.05431034]]
            print("type(Y): ", type(Y))
            ...
            try:
                ...
                # fps_pred tensor([[0.5745, 0.4335, 0.1862, 0.0536],
        #[0.5719, 0.4659, 0.2884, 0.0746]], device='cuda:0',
       #grad_fn=<AddmmBackward>)
                #print("y_pred", y_pred)
 
                #print("type(y_pred): ", type(y_pred))
                # type(y_pred):  <class 'list'>
            ...
            for j in range(7):  #Cross entropy loss for each number plate
            ...
            if i % 50 == 1:
                ...
                torch.save(model.state_dict(), storeName+'_'i+'_'+'.pth')
                print('%s %s %s\n' % (epoch, sum(lossAver) / len(lossAver), time() - start))
        model.eval()
        count, correct, error, precision, avgTime = eval(model, testDirs)
        with open(writeFile, 'a') as outF:
            outF.write('%s %s %s\n' % (epoch, sum(lossAver) / len(lossAver), time() - start))
            outF.write('*** total %s correct %s error %s precision %s avgTime %s\n' % (count, correct, error, precision, avgTime))
        print('*** total %s correct %s error %s precision %s avgTime %s\n' % (count, correct, error, precision, avgTime))
        torch.save(model.state_dict(), storeName + '_epoch_'+str(epoch)+'.pth')
    return model
...
if __name__ == '__main__':
    ...
    #batchsize = 1
    ap.add_argument("-w", "--writeFileForOutput", default='fh02.out', help="file for output")
    ap.add_argument("-t", "--test", required = True, help="Test images")
    ap.add_argument("-p", "--wR2Path", required = True, help="wR2 Path")
    ap.add_argument("-m", "--storeModel", required = True, help="storeModel")
    ...
    # About 2000 images
    #images = './ccpd_dataset/ccpd_base/images4'
    #epochs = 1
    
    #batch_size = 2
    #test = './ccpd_dataset/ccpd_base/images4'
    #storeModel = './rpnet_train_output'
    #writeFile = './rpnet_train_output/fh02.out'
    epoch_start = 0
    #resume='111'
    images = str(args["images"])
    epochs = int(args["epochs"])
    batch_size = int(args["batchsize"])
    writeFile = str(args["writeFileForOutput"])
    wR2Path = str(args["wR2Path"])
    test = str(args["test"])
    storeModel = str(args["storeModel"])
    ...
    numClasses = 7  # The number of license plates is 7
