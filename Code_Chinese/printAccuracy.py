# -*- coding: utf-8 -*-
"""
Created on Thu Jul 29 18:05:09 2021
...
def getAccuracy(lpnPred, imgName):
    #imageName = ims[0].split( "/")[-1]
    actual = imgName.split("-")[-3]
    print(actual)
    numbers = actual.split("_")
    print(type(lpnPred))
...
for i, (XI, ims) in enumerate(testloader):
 ...
 #download the font and copy it in this directory "/usr/share/fonts/truetype" with folder name as shown in the picture that's how you can access the font.
        ...
        createVOC(input, ims[0].split( "/")[-1] ,str(sizeOfXI[3]),str(sizeOfXI[2]),str(sizeOfXI[1]),lpn,str(xminimum),str(yminimum),str(xmaximum),str(ymaximum))
        print(getAccuracy(labelpred, imageName))
