...
def train_model(model, criterion, optimizer, num_epochs=25):
    ...
    for epoch in range(epoch_start, num_epochs):
        ...
        for i, (XI, YI, ims) in enumerate(trainloader):
            # print('%s/%s %s' % (i, times, time()-start))
            YI = np.array([el.numpy() for el in YI]).T
            if use_gpu:
                x = XI.cuda(0)
                y = torch.FloatTensor(YI).cuda(0)
                y.requires_grad = False
            else:
                x = XI
                y = torch.FloatTensor(YI)
                y.requires_grad=False
            ...
            if len(y_pred) == batchSize:
                ...
                loss.backward()
                optimizer.step()
                torch.save(model.state_dict(), storeName+'.pth')
            if i % 50 == 1:
...
if __name__=='__main__':
    ...
    images = './proper_dataset/v3andv4/padded/image/Good/image'# Add the path to the input file
    epochs = 300
    batchsize = 4
    resumeFileForRetrain = "./weight/wR2_Atulya_epoch_11.pth"
    writeFileForOutput = "./AtulyaFineTune/33images.out"
    annotationsFolder = './proper_dataset/v3andv4/padded/image/Good/annotation'
...
#Adjust the learning rate during training. For every 5 epochs, the learning rate is multiplied by 0.1
    ...
    dst = ChaLocDataLoader(images.split(','), imgSize, annotationsFolder)
